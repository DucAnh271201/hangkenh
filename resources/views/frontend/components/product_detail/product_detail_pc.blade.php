<section class="product-detail-pc body pc">
    <div class="block-left">
        <div class="image-slider" style="border-right:none">
            <img src="{{ get_media($product, 'feature_image', 'image') }}" class="mb-3"
                 alt="{{ $product->title }}">
            @if(!empty($product->image_library))
                @foreach($product->image_library as $item)
                    <img src="{{ $item['image'] }}" class="mb-3" alt="{{ $product->title }}">
                @endforeach
            @endif
        </div>
    </div>
    <div class="block-right">
        <div class="product-details">
            <h2 class="product-name-detail">{{ $product->title }}</h2>
            <p class="product-brand">Thiết kế
                bởi {{ !empty($product->post_author_name) ? $product->post_author_name : 'NTK Sơn Vũ' }}</p>
            <div class="product-price-rating">
                <span class="product-price">${{ number_format($product->regular_price) }}</span>
