@push('script')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            new TomSelect('#attribute-select', {
                hideSelected: true, // ẩn các lựa chọn đã được chọn khỏi dropdown
                closeAfterSelect: true, // đóng dropdown sau khi chọn một lựa chọn
                persist: false,
                plugins: ['remove_button'], // sử dụng plugin remove_button để có nút xóa
            ...
            function validateForm(){
                let regularPrice = Dom.selectId('regular-price-input').val()
                let salePrice = Dom.selectId('sale-price-input').val()

                if(regularPrice.length > 0 && salePrice > 0 && salePrice > regularPrice){
                    toastr.error("Giá khuyến mại không được lớn hơn giá gốc")
                    return false;
                }
                return true
            }
            ...
            Dom.change('.image-input-libs', function (event){
                const files = event.target.files;
                console.log(files.length)

                Dom.select('.image-box-libs .image-libs').addClass('grid-template-columns-2')

                Dom.select('.image-box-libs img').remove()
                let html = '';
                for (let i = 0; i<=files.length - 1; ++i){
                    ...
                    html += `<img
                                ...
                                alt="" style=" height: 120px !important;">`;
                }
                Dom.select('.image-box-libs .image-libs').prepend(html)
